                                                      #--MANDATORY PROJECT- 02  -- ON IG CLONE DATABASE--#
                               #----------------------------------------------------------------------------------------------#
...
-- IN ORDER TO MAKE THE ER DIAGRAM GO TO MENU BAR > DATABASE > REVERSE ENGINEER or PRESS CTRL+R > FILL THE PASSWORD > CHOOSE THE DATABASE > THEN FOLLOW THE STEPS
...
select dayname(created_at) from users GROUP BY  dayname(created_at);            -- EXTRACTING DAY FROM CREATED_AT
                                 
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'sunday');                              -- USING WHERE CLAUSE SEPERATELY --
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'monday');
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'tuesday');
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'wednesday');
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'thursday');
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'friday');
(select count(id),dayname(created_at) as day from users where dayname(created_at) = 'saturday');
